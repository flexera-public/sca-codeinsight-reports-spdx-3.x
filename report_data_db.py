...
def get_remote_scanned_files(projectID, inventoryID=None):
    logger.info("Entering get_remote_scanned_files")
    remote_scanned_files_query = f"SELECT REMOTE_SCAN_FILE.ID_ AS fileId, REMOTE_SCAN_FILE.PATH_ AS filePath, REMOTE_SCAN_FILE.MD5_ AS fileMD5, REMOTE_SCAN_FILE.SHA1_ AS fileSHA1, GRP_FILES.GROUP_ID_ AS inInventory FROM PSE_REMOTE_SCANNED_FILES REMOTE_SCAN_FILE JOIN PSE_INVENTORY_GROUP_FILES GRP_FILES ON REMOTE_SCAN_FILE.ID_ = GRP_FILES.FILE_ID_ WHERE PROJECT_ID_ = {projectID} and GRP_FILES.GROUP_ID_ = {inventoryID};"
    result = db_runner.run_query(remote_scanned_files_query)
    logger.info(result)
    return result

def get_file_license_evidence(projectID, fileID):
    logger.info("Entering get_file_license_evidence")
    sql = f"SELECT PD.NAME_ AS LICENSE FROM PSE_SCANNED_FILES SF LEFT JOIN PSE_SCAN_RESULT_NONSCF SRN ON SRN.ID_ = SF.NONSCF_RESULT_ID_ LEFT JOIN PSE_LICENSE_MATCH LM ON SRN.ID_ = LM.RESULT_ID_ LEFT JOIN PDL_LICENSE PD ON LM.LICENSE_ID_ = PD.ID_ WHERE SF.PROJECT_ID_ = {projectID} AND SF.ID_ = {fileID} GROUP BY PD.NAME_;"
    return db_runner.run_query(sql)

def get_project_copyright_evidence(projectID, fileID):
    logger.info("Entering get_project_copyright_evidence")
    sql = f"SELECT SF.ID_ AS fileId, SF.PATH_ AS filePath, CTXT.TEXT_ AS COPYRIGHT FROM PSE_SCANNED_FILES SF LEFT JOIN PSE_SCAN_RESULT_NONSCF SRN ON SRN.ID_ = SF.NONSCF_RESULT_ID_ LEFT JOIN PSE_COPYRIGHT_MATCH CM ON SRN.ID_ = CM.RESULT_ID_ LEFT JOIN PSE_COPYRIGHT_TEXT CTXT ON CM.TEXT_ID_ = CTXT.ID_ WHERE SF.PROJECT_ID_ = {projectID} AND SF.ID_ = {fileID} AND CTXT.TEXT_ IS NOT NULL GROUP BY SF.ID_, SF.PATH_, CTXT.TEXT_ ORDER BY SF.PATH_ ASC;"
    return db_runner.run_query(sql)
...
def get_license_short_names_for_ids(license_ids):
    """
    Given a list of integer license IDs, return a dict mapping each ID to its
    preferred display name: SHORT_NAME_ if present, otherwise SPDX_LICENSE_IDENTIFIER_,
    otherwise NAME_.  IDs not found in PDL_LICENSE are omitted from the result.
    """
    logger.info("Entering get_license_short_names_for_ids")
    if not license_ids:
        return {}
    id_list = ", ".join(str(lid) for lid in license_ids)
    sql = (
        f"SELECT ID_ AS licenseId, NAME_ AS licenseName, "
        f"SHORT_NAME_ AS shortName, SPDX_LICENSE_IDENTIFIER_ AS spdxIdentifier "
        f"FROM PDL_LICENSE WHERE ID_ IN ({id_list});"
    )
    rows = db_runner.run_query(sql)
    result = {}
    if rows and isinstance(rows, list):
        for row in rows:
            lid = row.get("licenseId")
            if lid is None:
                continue
            # Prefer shortName > spdxIdentifier > licenseName
            name = (
                row.get("shortName")
                or row.get("spdxIdentifier")
                or row.get("licenseName")
            )
            if name:
                result[int(lid)] = name
    return result
